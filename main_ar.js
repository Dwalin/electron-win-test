...
//const DbPopulator = require('./dbpopulator.js');
...
const pokerThreadId = 13032;

let yellowWindow;
const yellowPSWindow = 2493616;

let blueWindow;
const bluePSWindow = 1968626;

let windowslaver = new Windowslaver(process.pid);
//let dbpopulator = new DbPopulator();
...
const launchStrip = async () => {
  ...
  yellowWindow.loadURL('https://app.jivaro.com/extension');
  yellowWindow.setIgnoreMouseEvents(true);
  ...
  blueWindow.loadURL('https://app.jivaro.com/extension');
  blueWindow.setIgnoreMouseEvents(true);
  bluebuff = blueWindow.getNativeWindowHandle();

  await windowslaver.initialize(pokerThreadId);
  //await dbpopulator.initialize();
